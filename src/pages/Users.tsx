...
const ROLES = [
  ...
  { id: 'oemlinternal', name: 'OEML' },
  { id: 'opvtinternal', name: 'OPVT' },
  { id: 'asalinternal', name: 'ASAL' },
]
...
export default function Users() {
      ...
      try {
        ...
        if (profiles && profiles.length > 0) {
          ...
          setUsers([{
            ...
            role: user.user_metadata?.role || 'oemlinternal',
  ...
  const getRoleColor = (role: string) => {
    const colors: { [key: string]: string } = {
      ...
      oemlinternal: 'bg-emerald-500/10 text-emerald-400',
      opvtinternal: 'bg-blue-500/10 text-blue-400',
      asalinternal: 'bg-amber-500/10 text-amber-400',
    }
  ...
  const canEditUsers = currentUser?.user_metadata?.role === 'superadministrator' || currentUser?.user_metadata?.role === 'administrator'
