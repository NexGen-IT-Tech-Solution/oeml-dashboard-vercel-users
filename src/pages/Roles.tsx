...
const ROLES: Role[] = [
  { id: 'superadministrator', name: 'Super Administrator', description: 'Complete 100% access to all features and configurations', permissions: ['dashboard', 'users', 'roles', 'analytics', 'settings', 'create_user', 'modify_user', 'delete_user', 'manage_all_users'] },
  { id: 'administrator', name: 'Administrator', description: 'Dashboard access, create/modify users, no SuperAdmin access', permissions: ['dashboard', 'users', 'analytics', 'settings', 'create_user', 'modify_user'] },
  { id: 'oemlinternal', name: 'OEML Internal', description: 'Dashboard Overview only with portal redirection access', permissions: ['dashboard', 'portals'] },
  { id: 'opvtinternal', name: 'OPVT Internal', description: 'Dashboard Overview only with portal redirection access', permissions: ['dashboard', 'portals'] },
  { id: 'asalinternal', name: 'ASAL Internal', description: 'Dashboard Overview only with portal redirection access', permissions: ['dashboard', 'portals'] },
]
...
export default function Roles() {
  const [currentUser, setCurrentUser] = useState<{ user_metadata?: { role?: string } } | null>(null)
  const [users, setUsers] = useState<UserProfile[]>([])
  const [loading, setLoading] = useState(true)
  const [showCreateModal, setShowCreateModal] = useState(false)
  const [editingUser, setEditingUser] = useState<string | null>(null)
  const [newUser, setNewUser] = useState({ email: '', full_name: '', role: 'oemlinternal', password: '' })
    ...
    const fetchData = async () => {
      try {
        ...
        if (profiles && profiles.length > 0) {
          ...
          setUsers([{ id: user.id, email: user.email || '', full_name: user.user_metadata?.full_name || user.email?.split('@')[0] || 'User', role: user.user_metadata?.role || 'superadministrator', status: user.email_confirmed_at ? 'active' : 'deactivated', type: 'internal', created_at: user.created_at }])
        }
      } catch (err) {
        console.error(err)
      } finally {
  ...
  const createUser = async () => {
    ...
    if (data.user) {
      ...
      setUsers([{ id: data.user.id, email: newUser.email, full_name: newUser.full_name, role: newUser.role, status: 'active', type: 'internal', created_at: new Date().toISOString() }, ...users])
    }

    setShowCreateModal(false)
    setNewUser({ email: '', full_name: '', role: 'oemlinternal', password: '' })
  }
  ...
  const getRoleColor = (role: string) => {
    const colors: { [key: string]: string } = {
      ...
      oemlinternal: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20',
      opvtinternal: 'bg-blue-500/10 text-blue-400 border-blue-500/20',
      asalinternal: 'bg-amber-500/10 text-amber-400 border-amber-500/20',
    }
    return colors[role] || 'bg-neutral-500/10 text-neutral-400 border-neutral-500/20'
  }

  const getRoleName = (role: string) => {
    const names: { [key: string]: string } = { superadministrator: 'Super Admin', administrator: 'Admin', oemlinternal: 'OEML', opvtinternal: 'OPVT', asalinternal: 'ASAL' }
  ...
  const canManageRoles = currentUser?.user_metadata?.role === 'superadministrator' || currentUser?.user_metadata?.role === 'administrator'
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <button onClick={createUser} disabled={!newUser.email || !newUser.password || !newUser.full_name} className="w-full bg-emerald-500 hover:bg-emerald-400 text-emerald-950 font-semibold rounded-xl py-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
